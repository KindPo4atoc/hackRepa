        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function createTableCards() {
                ...
                for (const table of Object.entries(tables)) {
                    ...
                    tableCard.id = `table-${table['1'].name}`;
                    
                    let tableHTML = `<div class="table-name">${table['1'].name}</div>`;
                    
                    console.log(table['1']);
                
                    for(let i = 0; i < table['1'].columns.length; i++)
                    {
                        let badges = '';
                        if (table['1'].types[i][0]==='PK') badges += '<span class="pk-badge">PK</span>';
                        if (table['1'].types[i][0]==='FK') badges += '<span class="fk-badge">FK</span>';
                        tableHTML += `
